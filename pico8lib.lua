...
local lineMesh = love.graphics.newMesh(128,nil,"dynamic")
...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
	...
	if cart_mode then
		_load(cart_requested or 'nocart.p8')
		run()
	end
end
...
function load_p8(filename)
	...
	if filename:sub(#filename-3,#filename) == '.png' then
		...
		local start = data:find("pico%-8 cartridge // http://www.pico%-8.com\r?\nversion ")
		if start == nil then
			error("invalid cart")
		end
		local next_line = data:find("\r?\n",start+#header)
		...
		local next_line = 1
		while next_line do
			local end_of_line = gfxdata:find("\r?\n",next_line)
		...
		local next_line = 1
		while next_line do
			local end_of_line = gffdata:find("\r?\n",next_line)
			if end_of_line == nil then break end
			end_of_line = end_of_line - 1
		...
		local next_line = 1
		while next_line do
			local end_of_line = mapdata:find("\r?\n",next_line)
		...
		local next_line = 1
		while next_line do
			local end_of_line = sfxdata:find("\r?\n",next_line)
			if end_of_line == nil then break end
			end_of_line = end_of_line - 1
		...
		local next_line = 1
		while next_line do
			local end_of_line = musicdata:find("\r?\n",next_line)
			if end_of_line == nil then break end
			end_of_line = end_of_line - 1
...
function reload(dest_addr,source_addr,len)
	if not cart_mode then error("Memory functions not supported in library mode.") end
